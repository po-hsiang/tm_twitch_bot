...
#
# 值刻意寫成 qualname 字串而不是 import 進來的函式：sheet_config 必須
# import 這個模組（它要重載指令集），反過來 import 就成了循環。
# 走 _load_function 這條既有的路，順便連參數注入與例外收斂都是同一套。
BUILTIN_COMMANDS: dict[str, tuple[str, str]] = {
    "!RELOAD": ("function", "tm_twitch_bot.commands.reload.reload"),
}
...
# ===== 重構前的舊路徑 =====
#
# 「指令集」把 Python 模組路徑寫在儲存格裡，所以第十一輪的目錄重構會讓那些
# function 型指令全部指向不存在的模組。那張表是頻道主手動維護的、也不在版控裡，
# 程式改完不可能同時改好它——所以這裡放一張新舊對照表：舊路徑照樣能用，
# 只是會在 log 留下一行警告，提醒去更新試算表。
#
# **這是過渡用的東西，不是永久設計。** 試算表全部改成新路徑之後，
# 把這張表連同 _translate_legacy() 一起刪掉（見 docs/CODE_REVIEW.md 第十一輪）。
LEGACY_MODULE_PATHS: dict[str, str] = {
    "tm_twitch_bot.ai_actions.tm_ai_agent": "tm_twitch_bot.commands.ai_chat",
    "tm_twitch_bot.ai_actions.duel": "tm_twitch_bot.commands.duel",
    "tm_twitch_bot.games.gold_rush_game": "tm_twitch_bot.commands.games.gold_rush",
    "tm_twitch_bot.games.guess_number_game": "tm_twitch_bot.commands.games.guess_number",
    "tm_twitch_bot.scripts.daily_food_picker": "tm_twitch_bot.commands.food",
    "tm_twitch_bot.scripts.daily_meme_picker": "tm_twitch_bot.commands.meme",
    "tm_twitch_bot.scripts.gacha_handler": "tm_twitch_bot.commands.gacha",
    "tm_twitch_bot.scripts.rank_system": "tm_twitch_bot.commands.ranking",
    "tm_twitch_bot.scripts.vip_system": "tm_twitch_bot.commands.vip",
    "tm_twitch_bot.scripts.role_system": "tm_twitch_bot.commands.profile",
    "tm_twitch_bot.svc_client.youtube": "tm_twitch_bot.commands.songs",
}


def _translate_legacy(qualname: str) -> str:
    """把重構前的模組路徑換成現在的，並留下一行警告。

    警告只會在每個舊路徑第一次被用到時出現一次——`_load_function` 有 lru_cache，
    同一個字串不會再進來。這剛好是想要的：每次重啟提醒一次，不會洗頻 log。
    """
    module_name, _, func_name = qualname.rpartition(".")
    new_module = LEGACY_MODULE_PATHS.get(module_name)
    if new_module is None:
        return qualname
    logger.warning(
        f"指令集指向重構前的路徑「{module_name}」，已自動轉到「{new_module}」。"
        "請更新 Google Sheets「指令集」工作表的「回應內容 / 函數名稱」欄"
    )
    return f"{new_module}.{func_name}"
...
def _load_function(qualname: str):
    ...
    qualname = _translate_legacy(qualname)
    ...
    if "." not in qualname:  # 呼叫當前腳本內的函數
        try:
            ...
            raise ValueError(
                f"dispatcher.py 腳本內找不到函數 {qualname}()"
            ) from e
...
def clear_function_cache() -> None:
    ...
    有八個單例，reload 會生出第二份類別與第二個實例（見 sheet_config 的說明）。
