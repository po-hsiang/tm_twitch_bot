...
`dispatch_command` 現有的 `if not COMMAND_SET: await load_command_set()` 還會在第一次派發時自動補載入——等於 9091 晚一點開起來也能自己恢復。
...
P0 與 P1 已全數結案（P0-7 為評估後決定不處理），只剩本輪新發現的 P1-37。剩下的依投報率排序：

1. **P1-37**（9091 沒開就啟動失敗）——本輪查證時新發現，是目前唯一還會讓 Bot **起不來**的路徑，也是唯一牴觸「微服務掛掉不影響主體」原則的地方
2. **P2-23**（`$inc` 增量更新）——與已完成的 P2-24 是同一主題的下半場，解決並行寫入的 lost update
3. **P3-32**（服務清單）——降級為 15 分鐘的文件工作，見上方改寫後的說明
4. **P3-31**（lint / format 工具鏈）——CI 已經在跑了，加 `ruff` 幾乎是免費的
5. **P3-35**（時區不一致）——`task_scheduler` 用本機時區，其餘用 UTC+8；目前都在同一台機器上所以看不出來
6. **P2-25 ~ P2-29、P3-33、P3-34、P3-36**——體感影響都很小，可以慢慢清
