...
| P2-20 | 沒有 CI | ✅ `e8c8c3b` |
| P1-12 | 「每日」快取其實是「重啟才失效」 | ⏸️ 大部分為刻意設計，已重新界定範圍 |

測試總數 89 項（88 passed + 1 xfailed），全部離線。

同場加映：釐清了「開台／關台事件偵測」與營運架構的取捨，
結論見 [附錄 A](#附錄-a營運架構手動啟動-vs-常駐服務)。這個決策同時影響 P0-7、P1-12、P1-13、P3-32。
...
仍待決定的是一個範圍小得多的問題——`!吃` 與 `!梗` 的粒度不一致：

- `food_cache` 是 `dict[user_id, str]`，**每個觀眾各自一道菜**
- `meme_cache` 是**單一全域字串**，第一個打「梗」的人決定了全頻道所有人的梗

這個不對稱看起來像是實作差異而非設計意圖。若「每人各自一則梗」才是想要的，`meme_cache` 需比照 `food_cache` 改成 dict；若「全頻道共享同一則梗」就是刻意的，那維持現狀即可，本項結案。
...
- 89 項測試全部離線，`uv run pytest` 約 0.5 秒完成
- 覆蓋 `command_dispatcher`、`greeter`、`role_system`、`level_and_job_system`、`message_controller`、`task_scheduler`、`vip_system`、`mongo_atlas` + `rank_system`、`log_utils`

尚未覆蓋（後續補齊建議順序）：`gold_rush` / `guess_number` 的金幣邊界、`http_utils` 的重試策略（適合搭配 `respx`）。

### P2-20 ✅ 沒有 CI

`.github/workflows/ci.yml`

兩輪修正累積了 89 項測試，卻沒有任何機制自動執行它們，回歸保護實際上只在本機生效。
...
1. **P1-10 + P1-11**（重試策略、錯誤訊息不外洩）——都在 HTTP 與指令回覆路徑上，適合一起做；`!排行` 現在不會 TypeError 了，但錯誤訊息仍會把 `http://localhost:9093/...` 這種內部拓樸噴進公開聊天室
2. **P2-30**（Sheets 短列）——已有 `xfail` 測試等著轉綠，修起來只有一行，而它會讓 Bot **啟動就掛**
3. **P3-32**（部署設定）——見附錄 A 的結論：這是目前手動啟動架構下**最大的單點風險**，四個微服務的啟動方式只存在於一台機器的記憶裡
4. **P2-23**（`$inc` 增量更新）——與已完成的 P2-24 是同一主題的下半場，解決並行寫入的 lost update
5. **P1-13**（graceful shutdown）——與 P3-32 同一個主題（可維運性），適合一起做
6. **P1-12 的殘留問題**——只剩「`!梗` 要不要比照 `!吃` 改成分人」這一個決定，需要頻道主拍板
...
1. **要記得手動開。** 忘了就整場沒有機器人。
2. **中途掛掉沒有人救。** 目前沒有 supervisor，Bot 死了就死了（相關：P1-13 連 graceful shutdown 都還沒有）。
3. **綁在一台機器上。** 電腦重灌、換機、搬家就得重新考古——**四個本機微服務（9091–9094）的啟動方式完全沒有被記錄**（P3-32）。這是目前整個架構最大的單點風險，比「要記得手動開」嚴重得多。
...
**建議：暫時不要改成常駐，但把 A 架構真正會痛的地方補起來。**

理由是——A 的劣勢裡真正嚴重的是第 2、3 點（掛掉沒人救、綁在一台機器上），而**這兩點都不需要改成常駐就能解**：

- 第 2 點：加一層 supervisor（可以樸素到只是一個 `while` 迴圈的 `.bat`），配合 P1-13 的 graceful shutdown
- 第 3 點：把四個微服務的啟動方式記錄下來（P3-32），甚至只做容器化而不改成常駐——容器化與「常駐」是兩件事，可以只要前者
...
1. **先訂閱 `stream.online` / `stream.offline`，但只寫 log、不改任何行為。** 跑幾週，確認事件真的可靠、觀察抖動頻率
