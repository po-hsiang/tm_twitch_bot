...
**怎麼看這份文件**：前半是[待處理](#待處理)與兩份附錄（營運架構決策、測試覆蓋率），
後半是[已結案紀錄](#已結案紀錄)。第十一輪把兩者拆開，因為十輪修下來已結案的部分
佔了九成篇幅，待辦反而找不到。
...
第十輪之後，**P0、P1、P2 已全數結案**（P0-7、P1-12 為評估後決定不處理），
這份清單上只剩下面這一項。歷次修掉的東西全部搬到文件最後的
[已結案紀錄](#已結案紀錄)，這一區只留還沒解決的。
...
**為什麼它一直排在最後**：不是因為最不重要，而是因為它**不是純程式改動**。
要補得三件一起做——產生隨機 state、放進頻道主手動貼的授權網址、回來時比對。
那會改動頻道主的操作流程，得先確認要怎麼改才動手。
...
> 第十一輪的目錄重構讓這件事變得便宜很多：試算表現在只會指向 `commands/`
> 底下的模組（有測試守著），所以白名單就是「`tm_twitch_bot.commands.` 開頭」
> 一條規則，而不是要逐一列舉。

其餘是 VIP 名額計算撈回整張表、twitchio 2.x 已 EOL、YouTube 歌單快取無失效
機制、無自動重連（依營運決策刻意不做）。

**二、需要微服務配合的**：P2-23 的並行扣款——兩個流程同時扣款仍可能把 `gold`
扣成負數。要解得由 `mongo-atlas-svc` 支援條件式更新並回傳 `matched_count`，
那是 Go 那邊的工程。

**三、還沒實測的**：P0-3「他人兌換忠誠點數收不到事件」已修掉 EventSub 物件
被 GC 的疑似成因，但**還沒在正式頻道驗證**。下次開台請人兌換一次就能確認。

**四、過渡用的相容表**：第十一輪重構在 `chat/dispatcher.py` 留了
`LEGACY_MODULE_PATHS`，讓試算表上重構前的舊模組路徑照樣能用。
試算表更新完之後，把那張表連同 `_translate_legacy()` 與對應的測試一起刪掉。
...
`uv run pytest --cov` — **427 項測試，整體 79%**（1784 敘述句中 378 未覆蓋）。
...
| 層級 | 模組 | 覆蓋率 |
| --- | --- | --- |
| **完全覆蓋**（歷次修過的高風險模組） | `chat/sender`、`chat/greeter`、`commands/ai_chat`、`commands/food`、`commands/meme`、`commands/gacha`、`commands/reload`、`clients/n8n_ai_agent`、`config/loader`、`sheet_config`、`paths`、`utils/{singleton,time_utils,sheet_utils,error_utils}` | 100% |
| **高** | `utils/log_utils` 96%、`utils/token_manager` 95%、`commands/ranking` 94%、`model/character` 93%、`commands/games/guess_number` 92%、`chat/message_controller` 91%、`chat/dispatcher` 90% | 90–96% |
| **中** | `model/jobs` 88%、`utils/http_utils` 86%、`scheduler` 83%、`commands/games/gold_rush` 81%、`commands/vip` 80% | 80–88% |
| **偏低** | `utils/probability_utils` 75%、`clients/mongo_atlas` 65%、`commands/duel` 65%、`clients/openai` 62%、`clients/google_sheets` 54%、`commands/songs` 50%、`main` 41%、`clients/youtube` 41%、`commands/profile` 33%、`clients/twitch_vips_api` 24%、`bot` 23% | 23–75% |
| **零覆蓋** | `utils/dump_obj_utils` | 0% |
...
> **第十一輪的同一課，反過來的版本**：`main.py` 從 32% 變成 41%，聽起來像進步，
> 其實只是把沒測到的 `MyBot` 搬到 `bot.py`（23%）去了——**兩個檔案加起來未覆蓋的
> 行數完全一樣**。`commands/profile` 33%、`commands/songs` 50% 也是拆出來才「變差」的：
> 它們原本躲在 `role_system` 與 `youtube` 的分母裡。拆完之後補了 5 項測試把兩個
> 外殼的參數契約鎖住（`char` 與 `raw_tail_text` 這兩個名字就是契約）。
>
> 結論一樣：**看未覆蓋的絕對行數，不要看百分比的變化。**
...
**`bot.py` 23% 是合理的**：未覆蓋的是 `event_ready`、`on_points`、token 同步這些相依 twitchio 內部結構的部分，要測得先造一整套假的 twitchio。`main.py` 已覆蓋的則是真正有邏輯分支的 `shutdown()`；`load_sheet_config()`（降級啟動與自動恢復）第十一輪搬到 `sheet_config`，那支現在是 100%。
...
以下全部已經處理完（或評估後決定不處理），留著是為了「為什麼當時這樣決定」——
好幾個項目的結論和原本的建議相反，那些理由比修好的程式碼更值得留。
**要看還沒解決的請回到 [待處理](#待處理)。**

> 各項目寫的檔案路徑是**當時**的位置。第十一輪按職責重排了目錄
> （`scripts/` 拆成 `chat/`、`commands/`、`model/`；`svc_client/` 改名 `clients/`），
> 現在的位置見 README 的「專案結構」。刻意不回頭改寫這些路徑：那是當時的現場，
> 改了反而看不出「問題發生在什麼結構下」。
...
| 項目 | 狀態 |
| --- | --- |
| 拿掉 `src/` 那一層，專案路徑集中到 `paths.py` | ✅ |
| 按職責重排目錄，`commands/` 成為試算表唯一的指向對象 | ✅ 🧪 |
| `main.py` 拆成 `main.py`（啟動／收尾）＋ `bot.py`（事件處理） | ✅ |
| 本文件拆成「待處理」與「已結案紀錄」 | ✅ |

這一輪**沒有修任何缺陷**，是等價重構：一行業務邏輯都沒改，427 項測試全綠，
並實機確認線上 15 個 function 型指令全部還載得起來。

要解決的是「找不到東西」：`scripts/` 這個雜物櫃同時裝著訊息管線、資料模型、
指令派發、排程器與七個指令實作；更麻煩的是兩個層次錯位——`role_system.check`
（一個指令）住在資料模型裡，`youtube.pick`（兩個指令）住在 HTTP client 裡。

**新增的約定**：Google Sheets 的「指令集」把 Python 模組路徑寫在儲存格裡，
所以那是一份對外契約。現在規定它只能指向 `commands/` 底下，並有測試守著——
以後內部怎麼搬都不會再碰到那張表。過渡期用 `chat/dispatcher.py` 的
`LEGACY_MODULE_PATHS` 讓舊路徑照樣能用（每個舊路徑第一次被用到會 log 一行警告）。

測試總數 **427 項**，覆蓋率 **79%**。待處理仍是 1 項（P3-34）。
