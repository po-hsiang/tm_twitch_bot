...
測試總數 107 項（106 passed + 1 xfailed），全部離線。
...
1. **P2-30**（Sheets 短列）——已有 `xfail` 測試等著轉綠，修起來只有一行，而它會讓 Bot **啟動就掛**
2. **P3-32**（部署設定）——見附錄 A 的結論：這是目前手動啟動架構下**唯一真正嚴重的風險**，四個微服務的啟動方式只存在於一台機器的記憶裡
3. **P2-23**（`$inc` 增量更新）——與已完成的 P2-24 是同一主題的下半場，解決並行寫入的 lost update
4. **P1-15**（一桶金的錯誤訊息送不出去）——觀眾永遠看不到「沒有人參加」，體感上就是遊戲壞掉
5. **P1-14**（訊息速率與長度保護）——`send_to_channel()` 已經是現成的落點
6. **P1-13**（graceful shutdown）——依附錄 A 的結論已從「必要」降為加分項，但仍值得做
...
3. **綁在一台機器上。** 電腦重灌、換機、搬家就得重新考古——**四個本機微服務（9091–9094）的啟動方式完全沒有被記錄**（P3-32）。**這是目前整個架構唯一真正嚴重的風險**：前兩點有頻道主的習慣兜底，這一點沒有。
4. **長期任務無處可掛。** 「VIP 到期當天移除」「每日凌晨統計」這類需求做不了。
5. **開台前幾分鐘有空窗。** 觀眾比機器人早到。

> 頻道主的使用習慣（每日重開機、一定會記得開、單次故障可接受）等於免費消化掉了 A 的兩項劣勢。這讓 A 比原本評估的更划算——剩下唯一該補的是 P3-32。
...
**決定：維持 A 手動啟動**（2026-08-09，頻道主拍板），**唯一該補的是 P3-32**。

A 的五項劣勢裡，前兩項已被頻道主的使用習慣消化掉（每天重開機、一定會記得開、單次故障可接受），第 4、5 項是能接受的取捨，**只剩第 3 點（綁在一台機器上）是真的要處理的**——而它不需要改成常駐就能解：把四個微服務的啟動方式記錄下來（P3-32），甚至只做容器化而不改成常駐。容器化與「常駐」是兩件事，可以只要前者。

（P1-13 的 supervisor／graceful shutdown 因此從「必要」降為「加分項」——仍值得做，但不再是這條路線的前提。）
