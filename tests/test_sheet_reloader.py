...
def _admin(monkeypatch):
    monkeypatch.setitem(sr.config, "admin_user_id", [ADMIN])
...
async def test_a_regular_viewer_cannot_reload(spy_registry):
    ...
    assert await sr.reload(char=FakeChar(user_id="路人")) == ""
...
async def test_the_admin_reloads_every_sheet(spy_registry):
    ...
    reply = await sr.reload(char=FakeChar())
...
def test_the_startup_loader_list_is_shared_with_main():
    """main.py 的降級啟動與五分鐘重試必須吃同一份清單，不能各自維護。"""
    from tm_twitch_bot import main

    assert main.SHEET_LOADERS is sr.SHEET_LOADERS
