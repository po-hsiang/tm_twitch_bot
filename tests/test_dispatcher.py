...
from tm_twitch_bot.commands import reload as reload_cmd
from tm_twitch_bot.chat.dispatcher import dispatch_command
...
def spy_reload(monkeypatch):
    ...
    monkeypatch.setattr(reload_cmd, "reload", _fake_reload)
    return calls
...
# ===== 重構前的舊路徑（第十一輪）=====
#
# 「指令集」把 Python 模組路徑寫在儲存格裡，所以目錄重構會讓線上的 function 型
# 指令全部指向不存在的模組。那張表不在版控裡、也只能手動一格一格改，
# 所以 dispatcher 留了一張新舊對照表。這一段測試就是那張表的安全網。

# 重構當下線上指令集裡的 15 個 function 目標（2026-08-22 從 9091 撈出來的快照）。
# 試算表全部改成新路徑之後，這一段連同 LEGACY_MODULE_PATHS 一起刪掉。
LIVE_SHEET_TARGETS_BEFORE_RESTRUCTURE = [
    "tm_twitch_bot.ai_actions.duel.pk",
    "tm_twitch_bot.ai_actions.tm_ai_agent.ask",
    "tm_twitch_bot.games.gold_rush_game.start",
    "tm_twitch_bot.games.gold_rush_game.toss",
    "tm_twitch_bot.games.guess_number_game.guess",
    "tm_twitch_bot.games.guess_number_game.start",
    "tm_twitch_bot.scripts.daily_food_picker.pick",
    "tm_twitch_bot.scripts.daily_meme_picker.pick",
    "tm_twitch_bot.scripts.gacha_handler.gacha",
    "tm_twitch_bot.scripts.rank_system.top_heroes",
    "tm_twitch_bot.scripts.rank_system.top_richest",
    "tm_twitch_bot.scripts.role_system.check",
    "tm_twitch_bot.scripts.vip_system.redeem",
    "tm_twitch_bot.svc_client.youtube.pick",
    "tm_twitch_bot.svc_client.youtube.search_song",
]


@pytest.mark.parametrize("qualname", LIVE_SHEET_TARGETS_BEFORE_RESTRUCTURE)
def test_every_live_sheet_target_still_resolves(qualname):
    """重構不能讓線上任何一個指令變成「這個指令暫時出了點問題」。"""
    func = cd._load_function(qualname)
    assert callable(func)


@pytest.mark.parametrize("qualname", LIVE_SHEET_TARGETS_BEFORE_RESTRUCTURE)
def test_the_legacy_path_resolves_to_exactly_the_same_function(qualname):
    """舊路徑不只是「能載入」，載到的必須是同一個函式物件。"""
    new_qualname = cd._translate_legacy(qualname)

    assert new_qualname != qualname  # 這 15 個都應該被翻譯
    assert cd._load_function(qualname) is cd._load_function(new_qualname)


def test_using_a_legacy_path_leaves_a_warning(caplog):
    """log 要講清楚「哪個舊路徑、轉去哪、該去改哪裡」。"""
    with caplog.at_level(logging.WARNING):
        cd._load_function("tm_twitch_bot.scripts.gacha_handler.gacha")

    assert "tm_twitch_bot.scripts.gacha_handler" in caplog.text
    assert "tm_twitch_bot.commands.gacha" in caplog.text
    assert "指令集" in caplog.text


def test_an_unknown_path_is_left_alone():
    """對照表只該處理表上有的那幾個，其他一律原樣傳下去。"""
    assert cd._translate_legacy("shlex.split") == "shlex.split"
    assert cd._translate_legacy("本地測試函式") == "本地測試函式"


def test_everything_the_sheet_points_at_lives_under_commands():
    """這是 commands/ 這個套件的存在意義，也是這次重構的收斂點。

    試算表把 Python 模組路徑寫在儲存格裡，所以「它可以指向哪裡」是一份對外
    契約。全部收斂在 commands/ 之後，內部怎麼搬都不會再碰到那張表——
    這次會撞到的兩個地方（`role_system.check` 在資料模型裡、`youtube.pick`
    在 HTTP client 裡）就是因為指令散落在各層才會發生。
    """
    for old_module, new_module in cd.LEGACY_MODULE_PATHS.items():
        assert new_module.startswith("tm_twitch_bot.commands."), (
            f"{old_module} 轉到了 {new_module}，那不在 commands/ 底下"
        )


def test_the_builtin_command_also_lives_under_commands():
    for resp_type, qualname in cd.BUILTIN_COMMANDS.values():
        assert resp_type == "function"
        assert qualname.startswith("tm_twitch_bot.commands.")
